...
|---|---|---|---|
| P0 | 근로소득공제 | 별도 계산 슬라이스 | 총급여에서 근로소득금액을 산출하는 기본 단계 | ★ ### (3-4) 정책 클래스 상수 제거 해야함 
| P0 | 실제 종합소득 과세표준 세율표 | 별도 계산 슬라이스 | 현재 단순 세율 구현을 실제 세율표 기반으로 보강 |
| P0 | 근로소득세액공제 | 별도 계산 슬라이스 | 산출세액 이후 최종세액에 직접 영향 |
